...
| **The exact same confirmed announcement repeated every ~6 seconds indefinitely** for a stationary object (e.g. "Warning. Door ahead. It's very close." 4x in 24 seconds) - a short per-key cooldown limits repeat *rate*, not repetition itself. Directly reproduced live, then fixed and re-verified live. A first pass used a longer tier-scaled interval (12-30s) instead of a hard stop; a user report of continued repetition prompted taking Section 10's own example ("Person ahead. Then silence. If nothing changes, do not repeat.") literally instead. | Auto/navigation/color announcements now track the last *situation* (mode+label+zone+tier) spoken. A genuinely new one (different label, zone, or escalating tier) still announces immediately. An UNCHANGED situation now goes **fully silent** except CRITICAL, which still gets one reminder every `CRITICAL_REPEAT_SECONDS` (15s) - an ongoing close/blocking hazard is worth repeating; nothing else is. Live-verified: a non-critical announcement now says its piece once and stays quiet; a CRITICAL one (e.g. a very-close door) reminds every ~15s, not every ~6s. |
| A user reported hearing `"Person ahead."` in Navigation mode - investigated by directly inspecting the trained weights: **neither the indoor (10 classes) nor household (93 classes) model has a `person` class at all.** Confirmed the current code cannot produce that phrase (both models' class lists checked directly via `YOLO(...).names`, and live re-testing in Navigation mode over several minutes never produced it). Almost certainly a memory of pre-fix testing, when the old `"object"` mode's generic COCO model (which does have a `person` class) or the since-killed zombie process were still in play. | No code change needed - confirmed as already fixed by the COCO-removal work above; documented here so the finding isn't lost. |
